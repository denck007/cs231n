...
class crp_a_soft(object):
  ...
  def __init__(self, input_dim=(3, 32, 32),
               num_conv=2, num_filters=[2,2], filter_size=[3,3], conv_stride=[1,1],
               pool_size=[2,2], pool_stride=[2,2],
               num_affine=1, hidden_dim=[100], num_classes=10, weight_scale=1e-4, reg=0.0,
               dtype=np.float32):
    """
    ...
    for layer in range(num_conv): # initialize all the convolution weights
        ...
        self.params['W'+str(layer_num)] = np.random.randn(num_filters[layer],
                                                          up_c,
                                                          filter_size[layer],
                                                          filter_size[layer])*weight_scale
